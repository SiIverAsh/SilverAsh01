...
//findViewById()获取三个控件（R.id.push等）
        findViewById(R.id.push).setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                String addressStr = "dfafafdadfadfs";
        ...
        findViewById(R.id.image).setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
               // PictureSelector 实现图片选择功能。penGallery(SelectMimeType.ofImage())：打开图片选择器，并指定选择的媒体类型为图片。setSelectionMode(SelectModeConfig.SINGLE)：设置选择模式为单选，即只能选择一张图片。setImageEngine(GlideEngine.createGlideEngine())：设置图片加载引擎为 GlideEngine，使用 Glide 库加载和显示图片。
                PictureSelector.create(AdminAddHouseActivity.this).openGallery(SelectMimeType.ofImage()).setSelectionMode(SelectModeConfig.SINGLE).setImageEngine(GlideEngine.createGlideEngine()).setSandboxFileEngine(new UriToFileTransformEngine() {
                    @Override
                    ...
                    //启动图片选择器，设置一个 OnResultCallbackListener 来接收选择的结果
                }).forResult(new OnResultCallbackListener<LocalMedia>() {
                    @Override
                    public void onResult(ArrayList<LocalMedia> result) {
                        LocalMedia localMedia = result.get(0);
                        compressPath = localMedia.getAvailablePath();
                        Log.d("---------------", "onResult: "+compressPath);
                        GlideEngine.createGlideEngine().loadImage(AdminAddHouseActivity.this, localMedia.getAvailablePath(), imageview);
                    }
