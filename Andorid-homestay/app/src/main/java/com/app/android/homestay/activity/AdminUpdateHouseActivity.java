...
//                        });
                ...
                PictureSelector.create(AdminUpdateHouseActivity.this).openGallery(SelectMimeType.ofImage()).setSelectionMode(SelectModeConfig.SINGLE).setImageEngine(GlideEngine.createGlideEngine()).setSandboxFileEngine(new UriToFileTransformEngine() {
                    @Override
                    ...
                    public void onResult(ArrayList<LocalMedia> result) {
                        ...
                        GlideEngine.createGlideEngine().loadImage(AdminUpdateHouseActivity.this, localMedia.getAvailablePath(), image);
                    }
...
//获取传递过来的房屋信息对象，并根据该对象初始化界面上的房屋介绍、原价、折扣价、地址和图片等控件。
    @Override
...
//方法用于初始化省市区数据，其中从 assets 目录下的 province.json 文件中读取省市区数据，并解析为 JsonBean 对象列表。
...
//方法用于显示地址选择器。在选择完成后，将选中的地址信息显示在对应的 TextView 上。
